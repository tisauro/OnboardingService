...
async def revoke_device_certificate(certificate_id: str) -> None:
    ...
    principals_response = iot_client.list_thing_principals(
        thingName=
        iot_client.describe_certificate(certificateId=certificate_id)['certificateDescription']['certificateArn'].split(
            '/')[-1]  # This is a bit of a hack to get the thing name
    )
