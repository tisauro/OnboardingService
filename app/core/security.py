...
def validate_bootstrap_key(db: Session, key: str) -> models.BootstrapKey | None:
    ...
    keys = db.query(models.BootstrapKey).filter(models.BootstrapKey.is_active == True).all()

    for db_key in keys:
        if verify_password(key, db_key.key_hash):
            # Found a match. Now check expiration.
            if (
                    db_key.expiration_date and
                    db_key.expiration_date < datetime.datetime.now(datetime.timezone.utc)
            ):
                # Key is expired
                return None
